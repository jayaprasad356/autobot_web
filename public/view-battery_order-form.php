...
// session_start();
$order_id = $_GET['id'];
$sql = "SELECT *,battery_bookings.id AS id,battery_bookings.status AS status,users.name AS name,users.address AS address,users.pincode AS pincode,users.mobile AS mobile,battery_bookings.price AS price FROM `battery_bookings`,`batteries`,`users` WHERE battery_bookings.user_id=users.id AND battery_bookings.product_id=batteries.id  AND battery_bookings.id = '$order_id'";
$db->sql($sql);
$res = $db->getResult();
?>
<section class="content-header">
    <h1>View Battery Orders</h1>
                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        <table class="table table-bordered">
                            ...
                            <tr>
                                <th style="width: 200px">Battery Type</th>
                                <td><?php echo $res[0]['type'] ?></td>
                            </tr>
                            <tr>
                                <th style="width: 200px">Warranty</th>
                                <td><?php echo $res[0]['warranty'] ?></td>
                            </tr>
                            <tr>
                    ...
                    if (isset($_POST['btnUpdate'])) {
                        ...
                        $status = $db->escapeString($_POST['status']);    
                            ...
                            $sql = "UPDATE battery_bookings SET `status` = '$status' WHERE id = '" . $order_id . "'";
                            $db->sql($sql);
                    ...
                    $sql_query = "SELECT status FROM battery_bookings WHERE id = '" . $order_id . "'";
                    $db->sql($sql_query);
...
<script>
    ...
    function showpanel() {     
        window.location.replace("battery_orders.php");
    }
</script>
